...
} from './normalization-configs-archive.service'

@NestJSController('api/v1/normalization-config-archive')
export class Controller {
  constructor(private readonly normalizationConfigArchiveService: Service) {}
  ...
  run(@Param('id') id: string): Promise<void> {
    return this.normalizationConfigArchiveService.run({ id })
  }
  ...
  getById(@Param('id') id: string): Promise<NormalizationConfig> {
    return this.normalizationConfigArchiveService.getUnique({ id })
  }
  ...
  findFirst(
    ...
    return this.normalizationConfigArchiveService.findFirst(params)
  }
  ...
  async findAndCountMany(
    ...
    const [items, total] = await this.normalizationConfigArchiveService.findAndCountMany(params)
